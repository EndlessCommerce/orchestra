...
from orchestra.workspace.repo_tools import create_repo_tools
...
class TestToolNaming:
    ...
    def test_multi_repo_tool_names(self, repo_dir: Path, tracker: WriteTracker) -> None:
        ...
        names = {t.name for t in tools}
        assert "backend:read-file" in names
        assert "frontend:read-file" in names
...
class TestReadFile:
    def test_reads_file(self, repo_ctx: RepoContext, tracker: WriteTracker) -> None:
        tools = create_repo_tools({"project": repo_ctx}, tracker)
        read_tool = next(t for t in tools if t.name == "project:read-file")
...
class TestWriteFile:
    def test_writes_file(self, repo_ctx: RepoContext, tracker: WriteTracker) -> None:
        tools = create_repo_tools({"project": repo_ctx}, tracker)
        write_tool = next(t for t in tools if t.name == "project:write-file")
...
class TestEditFile:
    def test_edits_file(self, repo_ctx: RepoContext, tracker: WriteTracker) -> None:
        tools = create_repo_tools({"project": repo_ctx}, tracker)
        edit_tool = next(t for t in tools if t.name == "project:edit-file")
...
class TestSearchCode:
    def test_searches_repo(self, repo_ctx: RepoContext, tracker: WriteTracker) -> None:
        tools = create_repo_tools({"project": repo_ctx}, tracker)
        search_tool = next(t for t in tools if t.name == "project:search-code")
