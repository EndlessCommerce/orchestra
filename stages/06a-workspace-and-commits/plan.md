...
- [ ] Create `tests/integration/test_cxdb_agent_turns.py`
  ...
  - [ ] Test: AgentTurnCompleted emitted for every turn (with and without workspace)
...
- [ ] Create `tests/integration/test_workspace_e2e.py`
  - [ ] Test: full lifecycle — pipeline starts, branches created, agent modifies files across turns, per-turn commits with metadata, AgentTurn CXDB turns with SHAs, pipeline completes, original branches restored, session branches still exist
  - [ ] Test: multi-repo pipeline — 2 repos, separate branches and commits per repo
  - [ ] Test: pipeline failure mid-run — original branches still restored via finally block
  - [ ] Test: no workspace configured — AgentTurnCompleted still emitted, CXDB recording works, no git operations
