...
ambiguity:
  ...
  rationale:
    ...
    - "'orchestra attach' deferred from Stage 7 — removes the most complex new-infrastructure component"
    ...
    - "Remaining ambiguity: minimal — all major components well-specified"
...
complexity:
  ...
  rationale:
    ...
    - "'orchestra attach' deferred — removes the most complex new-infrastructure component from the capstone"
...
decision_points:
  - id: attach-ipc-mechanism
    ...
    resolved: true
    resolution: "DEFERRED — Attach command removed from Stage 7 scope. The Unix socket IPC infrastructure introduces new infrastructure rather than validating composition of existing features. Can be implemented in a future stage."
...
success_criteria:
  - id: tool-handler-tests
    description: All tool handler tests pass
    command: pytest tests/test_tool_handler.py -v
  ...
  - id: custom-handler-registration
    ...
    command: pytest tests/test_tool_handler.py -v -k custom
  ...
  - id: real-llm-integration
    ...
    command: ORCHESTRA_REAL_LLM=1 pytest tests/test_pr_review_real.py -v
...
**Attach command**: Deferred from Stage 7. The Unix socket IPC infrastructure introduces significant new infrastructure rather than validating composition of existing features. Can be implemented in a future stage.
...
**All features equal priority**: No features are deprioritized — tool handler, replay enhancement, status detail, PR review pipeline, observability, and cross-feature integration tests are all in scope and equally important.
...
### In Scope
- Tool handler (parallelogram shape, `tool_command` attribute, `shell=True` with documented risk, per attractor 4.10)
- CLI `orchestra replay --checkpoint` (extend existing replay command)
- CLI `orchestra status --detail` (rich CLI view replacing CXDB UI renderers)
- Adversarial PR review pipeline (DOT file + 4 new agent YAML configs, existing prompts retained)
...
- `orchestra attach` command (deferred — introduces new IPC infrastructure rather than validating composition)
